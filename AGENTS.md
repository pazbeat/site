...
- **Оплата**: `PAYMENT_MOCK=1` в `.env` включает демо-провайдера (страница `/pay/mock` шлёт подписанный вебхук) — для dev/e2e; в production не включать. **Kaspi Pay через шлюз PayQR (payqr.kz), боевой, без вебхуков**: `lib/payments/kaspi.ts` — `createInvoice` (`POST /v1/le/qr_invoice` c `machid`+`terNumber`+`orderid`(UUID)+`name`+`price` в тиынах строкой `${kzt}00`) возвращает `twocode` (ссылка `kaspi.kz/pay/PayQR`); `checkStatus` (`POST /v1/le/pay_status` `{orderid,machid}`, `code/status.code==="1"`→оплачено, `"2"`→ждём). createPayment не редиректит на внешний сайт, а на нашу `/{locale}/pay/kaspi?order=…`; страница создаёт инвойс (`/api/payments/kaspi/invoice`, QR из twocode) и опрашивает `/api/payments/kaspi/status` (при оплате → `fulfillOrder`, редирект на `/success`). Вебхуков нет → опрос клиентом (страница открыта). **TODO до боевого запуска: фоновый поллер pending-kaspi-заказов** — иначе оплата при закрытой странице не исполнится. **ВЫВЕРЕНО по рабочему Node-RED заказчика (2026-07-14):** наш Kaspi-код байт-в-байт совпадает с их рабочим (machid `IMBIRVEND`, terNumber `170340025554`, price `${kzt}00`, база `payqr.kz/v1/le`), invoice создаётся живьём (HTTP 200, `code:"1"`, `twocode`). Ошибка «аппарат недоступен к оплате» — на стороне Kaspi-мерчанта (service_id 7702 не активирован приём), НЕ в коде. Причём их боевой сайт Kaspi QR так и не завёл — реально платят через **ForteBank** (`POST api.fortebank.com/order`, Basic auth → `{id,hppUrl,password}`, оплата на hosted-странице `hppUrl?password=…&id=…`, статус `GET /order/{id}`). Если Kaspi не активируют — реализовать провайдер ForteBank (нужны их Basic-креды). Freedom Pay — каркас. Локальная БД — Docker-контейнер `imbir-pg` (postgres:16-alpine, user/pass/db: imbir). pg-boss стартует через `instrumentation.ts` (cron `expire-orders` каждые 5 мин).
