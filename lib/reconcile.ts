 ...
 *
 * У доставки он ВЫШЕ, чем у очереди (pg-boss делает свои пять попыток с
 * нарастающей паузой). Иначе счётчик исчерпывался бы ещё до того, как сверка
 * вообще получает право вмешаться: она смотрит на сертификат через пятнадцать
 * минут, а к этому моменту очередь уже израсходовала весь лимит, и чинились бы
 * только те письма, которые в очередь не попали вовсе.
 */
const MAX_ATTEMPTS = { altegio: 5, delivery: 10, repair: 5 };
...
export async function retryStuckDeliveries(
  ...
  // Отдельно — те, у кого попытки уже кончились. Прежде оповещение стояло
  // только внутри цикла по той же выборке, а она такие сертификаты как раз
  // исключает: сообщение о том, что попытки исчерпаны, не могло прийти
  // никогда. Именно эти случаи и требуют человека.
  const exhausted = await prisma.certificate.findMany({
    where: {
      sentAt: null,
      createdAt: {
        lt: threshold,
      ...
      deliveryAttempts: { gte: MAX_ATTEMPTS.delivery },
      OR: [{ scheduledAt: null }, { scheduledAt: { lt: threshold } }],
      order: { status: "paid" },
    },
    select: { id: true, serial: true, deliveryLastError: true },
    take: 10,
  });
  for (const cert of exhausted) {
    void reportFailure(
      "доставка: попытки кончились, сертификат у покупателя не появился",
      new Error(cert.deliveryLastError ?? "причина не записана"),
      { сертификат: cert.id, серийник: cert.serial },
    );
  }
  ...
  for (const order of orders) {
    ...
    void reportFailure(
      ...
      new Error(`заказ ${order.id}, ${order.amountKzt} ₸`),
      {
        ...
        // Инструкция, а не «нужен разбор»: сертификат погашен у нас, но в
        // Altegio продажа осталась, и кассир по номеру всё ещё найдёт живой
        // сертификат. Программно снять его в CRM мы не умеем — этот вызов не
        // проверен живьём, и выдумывать его на деньгах нельзя.
        "что сделать":
          "снимите сертификат в Altegio вручную — у нас он уже погашен, в CRM ещё нет",
        внимание: spent.length
          ? "часть суммы уже потрачена в салоне — услуга оказана, а денег за неё нет"
          : undefined,
      },
    );
  }
