 ...
 * дожим по историческим заказам при первом включении, и совсем свежие (первые
 * два часа) — они могут оказаться оплаченными, см. MIN_AGE_MS.
 */

const HOUR_MS = 60 * 60_000;
const WINDOW_MS = 24 * HOUR_MS;
/**
 * Сколько ждём, прежде чем написать «вы не завершили покупку».
 *
 * Заказ протухает через 30 минут, но это ещё ничего не значит: оплата могла
 * пройти и просто не дойти до нас — страницу закрыли, ответ провайдера
 * задержался, наш сервер перезапускался. Фоновый добор оплат ходит ступенями
 * до двух недель и такой заказ воскрешает. Письмо через 15 минут после
 * протухания попадало ровно в этот зазор: человек заплатил, а мы ему пишем,
 * что он не заплатил. Два часа — с запасом на все быстрые пути.
 */
...
export function dueRecovery(
  ...
  return age >= MIN_AGE_MS && age <= WINDOW_MS;
}
