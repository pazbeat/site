...
async function createBoss(): Promise<PgBoss> {
  ...
  // Без повторов и с большим сроком жизни задания. По умолчанию pg-boss
  // считает задание протухшим через 15 минут и ставит его заново: длинный
  // проход (много починок, каждая с сетевым синком) породил бы второй
  // одновременный проход поверх первого — а он чинит те же записи.
  await boss.createQueue(RECONCILE, {
    retryLimit: 0,
    expireInSeconds: 55 * 60,
  });
