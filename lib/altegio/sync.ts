...
export async function syncCertificateToAltegio(
  ...
  for (let attempt = 1; attempt <= MAX_NUMBER_ATTEMPTS; attempt++) {
    ...
    // там письмо ждёт синка, и замена никому не видна. С появлением
    // автоповтора (lib/reconcile.ts) тот же код может выполниться через часы
    // ПОСЛЕ доставки: у покупателя на руках письмо и PDF с номером WM9012, а
    // мы бы переписали сертификат на WM9013. Кассир по предъявленному номеру
    // не нашёл бы ничего, и виноватым оказался бы покупатель.
    if (cert.sentAt) {
      const reason = new Error(
        `номер ${number} занят («${outcome.message}»), а сертификат уже ` +
          `отправлен покупателю — менять номер нельзя, нужен человек`,
      );
      await markFailed(reason);
      const { reportFailure } = await import("@/lib/alerts");
      void reportFailure(
        "Altegio: номер занят, а сертификат уже у покупателя",
        reason,
