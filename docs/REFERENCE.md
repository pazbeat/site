...
- Перевыпуск второго фактора без смены пароля:
  `npx tsx scripts/reset-totp.ts <email>` — печатает новый QR

### Сверка платежей (`/admin/сверка` → «Сверка»)

Экран отвечает на один вопрос: где деньги и сертификат разошлись. Четыре вида
расхождений и что делать с каждым:

| Что видно | Что это значит | Что делать |
|---|---|---|
| Оплачено, сертификата нет | Деньги приняты, покупатель ничего не получил | Ничего: сверка сама повторяет выпуск каждые 10 минут. Держится дольше получаса — открыть заказ и выпустить вручную |
| Сертификат при неоплаченном заказе | Сертификат есть, оплаты нет | Найти платёж в выписке. Не нашли — заблокировать сертификат в карточке |
| Не записано в Altegio | У покупателя сертификат есть, кассир его не найдёт | Повтор идёт сам до пяти раз; дальше смотреть причину в строке и заводить в CRM руками |
| Не доставлено получателю | Письмо не ушло | Повтор идёт сам; при постоянной ошибке проверить адрес в карточке сертификата |

Раз в сутки в 09:30 приходит сводка расхождений — теми же каналами, что и
уведомления о продажах. Молчит, когда всё сходится.

Отдельно в 09:00 карточные оплаты за последний месяц переспрашиваются у банка:
не отменил ли он платёж после выпуска. Отмена гасит сертификат и присылает
оповещение. **По Kaspi так нельзя** — в ответе бэкенда есть только признак
«оплачено», состояния «отменено» в нём нет.

### Сверка с банковской выпиской

«Продажи» → «Скачать платежи за период (CSV)». В выгрузке дата оплаты, номер
операции у провайдера, промокод, номер сертификата, филиал и пометка ручного
подтверждения. Период считается по дате ОПЛАТЫ, а не создания заказа — так же,
как платёж стоит в выписке.

Строка `НЕТ СЕРТИФИКАТА` в выгрузке означает расхождение, а не пустое поле.
...
- `create-admin.ts` — создать администратора (перезаписывает и пароль)
- `reset-totp.ts` — перевыпустить второй фактор, не трогая пароль
