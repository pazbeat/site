...
- Убрать из `.env`: `PAYMENT_MOCK`, `ADMIN_2FA_DISABLED` — оба не действуют при
  `NODE_ENV=production` (`lib/payments/mode.ts`, `lib/admin/verify.ts`), но
  хранить их незачем. **Проверено кодом, а не обещанием:** до 2026-09-04
  здесь стояло «в проде игнорируются», хотя `PAYMENT_MOCK` действовал и
  выключал фоновый добор оплат — на боевом сервере, полторы недели.
- **`PREVIEW_NO_2FA=1` — настоящее отключение 2FA, и оно работает в production**
  (`lib/admin/verify.ts:23`). Заводился для временного превью. Пока флаг стоит,
  вход в админку защищён только паролем, а из админки доступен ручной выпуск
  сертификатов. Снимать перед боевой работой; сейчас стоит на сервере.
