...
describe("ограничение писем о сбоях", () => {
  it("блокировка по заказу не съедает общий лимит", async () => {
    // Ради этого проверка и отметка разделены: если отмечать при первой же
    // удачной проверке, письмо, не ушедшее из-за второго ограничителя, всё
    // равно расходовало бы лимит первого — и следующий, уже ДРУГОЙ, сбой
    // промолчал бы 15 минут.
    const { canEmail, markEmailed } = await import("@/lib/alerts");
    const t = Date.UTC(2026, 8, 4, 12, 0, 0);

    expect(canEmail("вид-сбоя", t)).toBe(true);
    // Письмо не ушло (сработал ограничитель по заказу) — общий ключ не тронут
    expect(canEmail("вид-сбоя", t + 1000)).toBe(true);

    markEmailed("вид-сбоя", t + 1000);
    expect(canEmail("вид-сбоя", t + 2000)).toBe(false);
    expect(canEmail("вид-сбоя", t + 16 * 60_000)).toBe(true);
  });
});
